...
class LifetimeCalculator(QtGui.QWidget):
    ...
    def __init__(self, kappa2=0.667):
        ...
        self.kappa2 = kappa2
        ## User-interface
        self.connect(self.doubleSpinBox, QtCore.SIGNAL("valueChanged(double)"), self.onTau0Changed)
        self.connect(self.doubleSpinBox_2, QtCore.SIGNAL("valueChanged(double)"), self.onTauChanged)
        self.connect(self.doubleSpinBox_3, QtCore.SIGNAL("valueChanged(double)"), self.onR0Changed)
        self.connect(self.doubleSpinBox_9, QtCore.SIGNAL("valueChanged(double)"), self.onRChanged)
        self.connect(self.doubleSpinBox_4, QtCore.SIGNAL("valueChanged(double)"), self.onEChanged)
        self.connect(self.doubleSpinBox_5, QtCore.SIGNAL("valueChanged(double)"), self.onkFRETChanged)
        self.onkFRETChanged()
        self.hide()
