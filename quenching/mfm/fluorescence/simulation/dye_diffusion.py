...
import itertools
import mfm
from mfm.fitting.parameter import AggregatedParameters, Parameter
...
class DecaySimulationParameter(AggregatedParameters):
    ...
    def __init__(self, dt_mt=100.0, dt_tac=0.032, n_tac=4096, n_curves=4096, n_photons=10e6,
        ...
        self._dt_mt = Parameter(value=dt_mt, name='dtMT[ns]')
        self._dt_tac = Parameter(value=dt_tac, name='dtTAC[ns]')
        self._n_tac = Parameter(value=n_tac, name='nTAC')
        self._n_curves = Parameter(value=n_curves, name='n curves')
        self._n_photons = Parameter(value=n_photons, name='n photons')
        self._tac_range = kwargs.get('tac_range', (0, 50))
        # decay mode has to be either photons or curve
        self._decay_mode = decay_mode


class DyeDecay(mfm.Curve):

    def __init__(self, decay_parameter,
                 diffusion_simulation, **kwargs):
        self.fit = kwargs.get('fit', None)
        mfm.Curve.__init__(self, **kwargs)
