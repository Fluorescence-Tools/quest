...
@cython.boundscheck(False)
def simulate_traj(
        np.ndarray[dtype=np.uint8_t, ndim=3] av_density,
        np.ndarray[dtype=np.uint8_t, ndim=3] ds,
        double dg,
        double t_max,
        double t_step,
        double D,
    ...
    k = sqrt(diffusion_coefficient * dimensions * tau);
    ...
    cdef int32_t grid_points_per_axis = av_density.shape[0]
    ...
    # MCMC-integration
    pos[0, 0] = initial_position[0]
    pos[0, 1] = initial_position[1]
    pos[0, 2] = initial_position[2]
    ...
    for i in range(n_samples):
        ...
        if 0 < x_idx < grid_points_per_axis and 0 < y_idx < grid_points_per_axis and 0 < z_idx < grid_points_per_axis:
            if av_density[x_idx, y_idx, z_idx] > 0:
                i_accepted = i
                n_accepted += 1
                accepted[i] = True
            else:
                n_rejected += 1
                accepted[i] = False
    return (pos - (grid_points_per_axis - 1) / 2) * dg, accepted, n_accepted, n_rejected
