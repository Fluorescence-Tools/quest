...
from mfm.fitting.models.tcspc.fret import FRETrateModelWidget, WormLikeChainModelWidget, GaussianChainModelWidget
from mfm.fitting.parameter import ParameterWidget
from .tcspc import GenericWidget, CorrectionsWidget
from .tcspc import LifetimeModelWidget, SingleDistanceModelWidget
from mfm.widgets import clearLayout
from mfm.fluorescence import stack_lifetime_spectra
from mfm.fitting.fit import FittingWidget
from .tcspc import ConvolveWidget, FRETModel, LifetimeModel
...
class MixModelWidget(MixModel, QtGui.QWidget):
    ...
    model_types = [GaussianModelWidget, WormLikeChainModelWidget,
                   GaussianChainModelWidget, LifetimeModelWidget,
    ...
    def amplitude(self):
        ...
        for i in range(layout.count()):
            ...
            if isinstance(item, ParameterWidget):
                re.append(item)
        return re
    ...
    def __init__(self, fit, **kwargs):
        ...
        generic = GenericWidget(model=self, **kwargs)
    ...
    def add_model(self):
        ...
        name = "x(%s)" % (len(self) + 1)
        fraction = ParameterWidget(name=name, value=1.0, model=self, ub=1.0, lb=0.0, layout=l)
