...
import mfm.io
import mfm.math
from mfm import Structure, get_coordinates_of_residues, plots
import mfm.fluorescence.fps as fps
from mfm.widgets import PDBSelector
from mfm.fluorescence.fps import AV


class DyeDecay(Model, mfm.Curve):
    ...
    def av(self):
        ...
        """
        if not isinstance(self._av, fps.AV):
            self._av = self.calc_av()
            self._av.calc_slow_av(save=self.save_av, slow_centers=self.slow_center,
    ...
    def __init__(self, tau0=4.2, k_quench_protein=5.0, n_photons=10e6,
        ...
        self.convolve = kwargs.get('convolve', tcspc.Convolve(self.fit))
        self.generic = kwargs.get('generic', tcspc.Generic())
        self.corrections = kwargs.get('corrections', tcspc.Corrections(model=self, **kwargs))

        self.quencher = quencher
        self.verbose = mfm.verbose
    ...
    def calc_av(self, **kwargs):
        ...
        av = AV(structure, residue_seq_number=attachment_residue, atom_name=attachment_atom, chain_identifier=chain,
    ...
    def simulate_diffusion_trajectory(self, **kwargs):
        ...
        diffusion.update(D=self.diffusion_coefficient, slow_fact=self.slow_fact, t_max=self.t_max, t_step=self.t_step)
        return diffusion
    ...
    def save_histogram(self, filename='hist.txt', verbose=False, nbins=4096, range=(0, 50)):
        ...
        mfm.io.txt_csv.save_xy(filename, x, y, verbose, header="time\tcount")
    ...
    def update_model(self, **kwargs):
        ...
        self.av.calc_slow_av(save=self.save_av, slow_radius=self.slow_radius, slow_centers=self.slow_center)
...
class TransientDecayGenerator(QtGui.QWidget, DyeDecay):
    ...
    def __init__(self, fit, **kwargs):
        ...
        convolve = tcspc.ConvolveWidget(fit=fit, model=self, dt=fit.data.dt, **kwargs)
        corrections = tcspc.CorrectionsWidget(fit, model=self, **kwargs)

        settings_file = kwargs.get('dye_diffusion_settings_file', './settings/dye_diffusion.json')
        ...
        if not kwargs.get('disable_fit', False):
            fitting_widget = mfm.fitting.fit.FittingWidget(fit, **kwargs)
        else:
