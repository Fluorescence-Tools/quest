...
def centroid2(int[:,:] atom_lookup, int[:] res_types, double[:, :] ca_dist, double[:, :] r, float[:, :, :] potential,
       double cutoff=6.5, int c_beta_pos=4, double min_dist=3.5, double max_dist=19.0, double bin_width=0.05,
    ...
    cdef int nRes = atom_lookup.shape[0]
    cdef int bin_nbr
    cdef double E, dE
    cdef double xi, yi, zi, xj, yj, zj, dij
    cdef int nCont
    cdef int c_beta_atom_nbr_i, c_beta_atom_nbr_j
    cdef int residue_j, residue_i
    cdef int residue_type_i, residue_type_j

    E, nCont = 0.0, 0
    for residue_i in prange(nRes, nogil=True):
        c_beta_atom_nbr_i = atom_lookup[residue_i, c_beta_pos]
        ...
        for residue_j in range(residue_i + 1, nRes - 1):
            c_beta_atom_nbr_j = atom_lookup[residue_j, c_beta_pos]
