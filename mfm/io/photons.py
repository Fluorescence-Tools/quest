...
def spc2hdf(spc_files, routine_name="bh132", verbose=False, **kwargs):
    ...
    headertable = h5.create_table('/' + title, 'header', description=Header, filters=filters)
    header = headertable.row
    photontable = h5.create_table('/' + title, 'photons', Photon, filters=filters)
